...
def B_init(nodes, division_x, division_y):
    ...
    for node in nodes:
        ...
        B[node.node_id - 1] = (1 - node.f) * (node.a_west * nodes[node.west_node_id - 1].temperature[-1] + node.a_east * nodes[node.east_node_id - 1].temperature[-1] + node.a_south * nodes[node.south_node_id - 1].temperature[-1] +
                                              node.a_north * nodes[node.north_node_id - 1].temperature[-1]) + node.temperature[-1] * (node.a_persent_0 - (1 - node.f)*(node.a_west + node.a_east + node.a_south + node.a_north - node.S_P * division_x*division_y))+ node.b
    return B
...
if __name__ == '__main__':
    ...
    if not os.path.exists(work_path):
        os.mkdir(work_path)
    work_path = os.path.join(work_path, 'numpy')
    if not os.path.exists(work_path):
        os.mkdir(work_path)
    ...
    node_list = []
    for node in nodes:
        if node.north_node_id == 0 or node.east_node_id == 0:
            node_list.append(node.node_id - 1) 
 ...
 # 求解线性方程组
    with tqdm(total=n, desc='In iteration') as tbar:
        for i in range(n):
            ...
            B = B_init(nodes, division_x, division_y, node_list)
            Temperature =solve(A, B)
            [node.temperature.append(Temperature[node.node_id - 1]) for node in nodes]
            [node.time_step.append(time_p) for node in nodes]
            if i%1 == 0:
                data_write_csv(data_path+f"\\{time_p:.4f}s.csv",Temperature)
    ...
    # 加入tqdm的pbar，用于显示进度条
    pbar = tqdm(total=n, desc="Ploting")
    ...
    # 绘制GIF动态图
    img_list = []
    file_path = [f't={n*0.05:.4f}s.png'
                 for n in range(time_length*20+1)]
