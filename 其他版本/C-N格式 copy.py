...
class Node_info:
    ...
    def S_calculate(self, division_x, division_y, ):
        if self.north_node_id == 0 and self.east_node_id == 0:
            ...
            self.S_C = q * division_y + q * division_x
...
def generate_nodes(x_length, y_length, division_x, division_y):
    ...
    for i in range(len(y_coords)):
        for j in range(len(x_coords)):
            ...
            node.x = np.round(node.x,6)
            node.y = np.round(node.y,6)
            nodes.append(node)
    return nodes
...
def B_init(nodes, division_x, division_y):
    ...
    return B
def B_init2(nodes, division_x, division_y, B):
    B_1 = B
    for node in nodes:
        node.S_calculate(division_x, division_y)
        node.b_p(division_x, division_y)
        B_1[node.node_id - 1] = B[node.node_id - 1]+ node.b
    return B_1
...
if __name__ == '__main__':
    ...
    with tqdm(total=n, desc='In iteration') as tbar:
        for i in range(n):
            ...
            B = B_init2(nodes, division_x, division_y, B_0)
